...
# ----------------------------------------
# Paste a script, optionally add topic keywords, click "Generate Video".
# The app creates narration with gTTS, finds a matching image per scene from Pexels,
# and stitches everything into a 720p video with MoviePy.
#
# Deployment: Streamlit Community Cloud (free). Set PEXELS_API_KEY in "Secrets".
...
# Ensure MoviePy knows where ffmpeg is (bundled by imageio-ffmpeg)
...
def split_into_scenes(script: str) -> List[str]:
    ...
    # Fallback: split by sentence end, then group ~3 sentences per scene
...
def safe_keyword_text(text: str, fallback: str) -> str:
    """Create a short keyword string for image search."""
    # Keep letters, numbers, spaces
    cleaned = re.sub(r'[^A-Za-z0-9\s]', '', text)
    cleaned = re.sub(r'\s+', ' ', cleaned).strip()
    # Use first 6 words, or fallback
    words = cleaned.split()
...
def fetch_image_from_pexels(query: str, dest_path: str) -> bool:
    """Search Pexels for a landscape image and save to dest_path. Returns True if saved."""
...
def make_placeholder_image(text: str, dest_path: str, size=(1280, 720)):
    """Create a simple placeholder image with wrapped text."""
...
def build_video(scenes: List[str], topic_keywords: str, output_path: str, fps: int = 24):
    """Create a 720p MP4 by pairing each scene's narration with a matching image."""
    clips = []
    ...
    for i, scene in enumerate(scenes):
        ...
        # --- Make 720p visual clip sized to audio duration ---
        # We letterbox to 1280x720 while preserving aspect ratio.
    ...
    # Try h264+aac, fallback to mpeg4 if needed
    try:
        final.write_videofile(output_path, fps=fps, codec="libx264", audio_codec="aac", threads=2, verbose=False, logger=None)
    except Exception:
        final.write_videofile(output_path, fps=fps, codec="mpeg4", audio_codec="aac", threads=2, verbose=False, logger=None)
